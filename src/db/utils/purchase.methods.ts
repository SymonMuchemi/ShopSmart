...
const calculateTotalFromPurchaseItems = async (
...
export const recordPurchase = async (
  ...
  try {
    ...
    if (isNaN(total))
      console.log(`Total is not a number; it is ${typeof total}`)
...
export const markPurchaseAsPaidOrDeclined = async (
  purchaseId: string,
  ...
  try {
    ...
    if (!purchase)
      throw new Error(`Error finding purchase record with ID: ${purchaseId}`)
...
const isProductPurchasable = async (
  productId: string,
  quantity: number,
): Promise<boolean> => {
...
export const getUserPurchaseHistory = async (
  ...
  try {
    ...
    throw new Error(
      `purchase.methods.getUserPurchaseHistory ${error.toString()}`,
    )
  }
}
